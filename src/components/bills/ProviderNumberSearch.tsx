...
type ProviderNumberSearchProps = {
  billType: string;
  value: string;
...
export const ProviderNumberSearch = ({
  billType,
  value,
  onChange,
  placeholder = "Saisir le numéro du fournisseur",
  required = false
}: ProviderNumberSearchProps) => {
  ...
  const searchProviderByNumber = async (searchNumber: string) => {
    ...
    try {
      ...
      if (data) {
        console.log("✅ Fournisseur trouvé:", data);
        setIsVerified(true);
        setFoundProviderName(data.provider_name);
        onChange(searchNumber, data.provider_name);
      }
    } catch (error) {
