...
export const AgentBalanceRechargeButton = () => {
  ...
  const handleRecharge = async () => {
    ...
    try {
      ...
      // Créer une recharge automatique pour l'agent
      const { error } = await supabase.from('recharges').insert({
        ...
        payment_phone: 'agent',
        country: 'Cameroun',
        transaction_reference: `AGENT_RECHARGE_${Date.now()}`,
        status: 'completed',
      ...
      // Créditer le solde via RPC sécurisé
      ...
      toast.success(`Recharge de ${rechargeAmount.toLocaleString()} XAF effectuée avec succès`);
      setAmount("");
      setIsOpen(false);
