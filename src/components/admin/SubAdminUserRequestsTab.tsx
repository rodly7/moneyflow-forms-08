    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold">Demandes Utilisateurs</h2>
        </div>
        <Button 
                ...
                <div key={request.id} className="border rounded-lg p-4 space-y-3 opacity-75">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div>
                      ...
                      <span>{request.processed_at ? new Date(request.processed_at).toLocaleDateString('fr-FR') : 'N/A'}</span>
                    </div>
                  </div>

                  {request.rejection_reason && (
                    <div className="text-sm bg-red-50 p-2 rounded">
                      <span className="text-red-600 font-semibold">Raison du rejet: </span>
          ...
          <div className="space-y-4">
            <div>
              <p className="text-sm text-muted-foreground mb-2">
                Vous êtes sur le point de rejeter cette demande. Veuillez préciser la raison:
              </p>
              <Textarea
                value={rejectionReason}
                onChange={(e) => setRejectionReason(e.target.value)}
                placeholder="Ex: Montant trop élevé, documents manquants..."
                rows={3}
              />
            </div>
...
export default SubAdminUserRequestsTab;