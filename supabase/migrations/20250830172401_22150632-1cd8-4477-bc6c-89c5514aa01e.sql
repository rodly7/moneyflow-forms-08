-- Supprimer l'ancienne vue et la recréer avec les bonnes données
DROP VIEW IF EXISTS user_requests CASCADE;
...
-- Note: Les vues héritent des politiques de leurs tables de base
-- Nous nous assurons que les sous-administrateurs peuvent voir les données

-- Mettre à jour les politiques sur recharges pour les sous-administrateurs
DROP POLICY IF EXISTS "Sub-admins can view recharges" ON recharges;
...
DROP POLICY IF EXISTS "Sub-admins can view withdrawals" ON withdrawals;
...
DROP POLICY IF EXISTS "Sub-admins can update recharge status" ON recharges;
CREATE POLICY "Sub-admins can update recharge status" ON recharges
  FOR UPDATE USING (is_admin_or_sub_admin(auth.uid()));

DROP POLICY IF EXISTS "Sub-admins can update withdrawal status" ON withdrawals;
