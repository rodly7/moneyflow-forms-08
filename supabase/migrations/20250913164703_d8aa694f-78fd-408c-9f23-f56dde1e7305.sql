-- Drop and recreate the find_recipient function with the correct signature
DROP FUNCTION IF EXISTS public.find_recipient(text);

-- Create a new function to safely search recipients by phone
-- Returns only non-sensitive fields and bypasses RLS for profiles
CREATE FUNCTION public.find_recipient(search_term text)
RETURNS TABLE (
  id uuid,
  full_name text,
  phone text,
