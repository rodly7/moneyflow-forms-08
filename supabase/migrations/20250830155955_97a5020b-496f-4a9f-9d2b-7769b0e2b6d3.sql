-- Mettre à jour la fonction de calcul des performances mensuelles
-- pour que les bonus soient basés uniquement sur les dépôts et retraits
...
DECLARE
  ...
  -- Variables pour les bonus basés uniquement sur dépôts et retraits
  deposits_withdrawals_volume NUMERIC := 0;
  deposits_withdrawals_count INTEGER := 0;
BEGIN
  ...
  -- Variables pour les bonus : UNIQUEMENT dépôts et retraits
  deposits_withdrawals_volume := withdrawals_volume + deposits_volume;
  deposits_withdrawals_count := withdrawals_count + deposits_count;
  ...
  -- Calculer les bonus BASÉS UNIQUEMENT SUR DÉPÔTS ET RETRAITS
  -- Bonus de transactions : minimum 100 transactions dépôts + retraits
  IF deposits_withdrawals_count >= 100 THEN
    transaction_bonus := 5000;
  END IF;
  
  -- Bonus de volume : minimum 1M FCFA en dépôts + retraits
  IF deposits_withdrawals_volume >= 1000000 THEN
    volume_bonus := 5000;
  END IF;
